...
i sprema ih u JSON (eng. JavaScript Object Notation) obliku. Naknadno su te JSON datoteke pročišćene i iskorištene za daljnje korištenje. Tijekom svoje Erasmus+ studentske mobilnosti imao sam prilike razgovarati s profesoricom na 
...
\begin{itemize}
    ...
    Pretjerana briga oko toga nema smisla jer Erasmus+ zahtijeva osnovno znanje stranog jezika i nije problem ako neko nije dobar u tome. Erasmus+ je upravo dobra prilika da se razbije 
...
Unutar ovog poglavlja opisana je važnost inteligentnih pomoćnih sustava te je napravljena podijela na asistente, copilote, agente i chatbotove. Svaki od njih je ukratko opisan uz pripadajuće 
primjere. Za kraj ovog poglavlja opisano je korištenje inteligentnih asistenata u sustavu obrazovanja, njihova primjena i koji su izazovi takvog korištenja.
...
njegovu mjeru performansi, s obzirom na: mjeru performansi, prethodno znanje, raspoložive akcije i dosadašnji niz percepata [\cite{russell2022ai}, pp. 58--59]. Racionalnost nije sveznanje 
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.7\textwidth]{./assets/ar3.png} 
...
U svijetu umjetne inteligencije često se pojavljuju pojmovi poput asistenta, copilota, autonomnih agenata i chatbot platformi. Iako na prvi pogled djeluju kao sinonimi, svaki od njih ima svoju 
...
\subsection{Copiloti - Microsoft Copilot Studio}
...
Primarno taj sustav je razvijen od strane Microsofta i koristi se za izradu copilota koji mogu odgovarati na specifična pitanja koja su usko vezana za neku domenu. U alatu se je isprobalo korištenje varijabli, entiteta, 
...
za pozadinske operacije (inicijalna poruka pri početku konverzacije ili završetak razgovora). Kada copilot nema definiranu temu za odgovor na neko pitanje onda odlazi u sistemske teme i tamo 
...
Chatbot platforme su sustavi temeljeni na kreiranim pravilima koji su osmišljena za izvršavanje određenih zadataka putem unaprijed definirane interakcije. Iako posjeduju dosta ograničenja jako se dobro
...
\begin{itemize}
    \item Tvrtke mogu smanjiti troškove korisničke podrške za 30\,\% korištenjem chatbot platforme,
...
Generator je važan jer pretvara neobrađene podatke u koherentan i strukturiran tekst i osigurava da odgovori zvuče prirodno i privlačno. Također, spaja unaprijed obučeno znanje o umjetnoj inteligenciji
...
Kroz sljedeću sekciju istražena je primjena RAG-a. Na početku istaknut je jedan primjer koji je podijelio Siddharth Asthana u jednom od svojih
...
U nastavku je navedeno nekoliko domena u kojoj se RAG može primjenjivati \cite{nishtha2025rag}:
...
LLM-ovi su ključna tehnologija umjetne inteligencije koja pokreće AI sustave. Priroda LLM-ova je takva da ponekad unose dozu nepredvidljivosti
...
korisnika da kontinuirano treniraju model na novim podacima i ažuriraju njegove parametre. Na taj način, RAG može smanjiti računalne i financijske troškove pokretanja AI sustava \cite{ibmRAG}.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.65\textwidth]{./assets/Multi hop.png}
...
Na sljedećoj slici može se vidjeti sažetak alata i biblioteka koje se mogu koristiti za implementaciju RAG sustava.
...
\subsubsection{FAISS}
FAISS (eng. Facebook AI Similarity Search) je robusna vektorska baza podataka koju je razvio Facebook-ov istraživačkim tim za umjetnu inteligenciju. Vrlo je učinkovita i brza, što je čini 
...
LangChaina, llama modela s Groq platforme i FAISS vektorske baze podataka. 
...
FastAPI-a koristi se port 8000 i može se testirati putem preglednika ili alata poput Postmana. 8000/docs putanja omogućuje pristup
...
Ova dokumentacija omogućuje testiranje svih dostupnih API poziva izravno kroz preglednik. Alternativno, dostupna je ReDoc dokumentacija na ruti /redoc,
koja prikazuje API specifikaciju na čitljiviji način, ali bez mogućnosti testiranja. Također, ruta /openapi,json prikazuje OpenAPI specifikaciju
...
Praktični dio rada se sastoji od izrade web stranice i izrade inteligentnog asistenta temeljenog na RAG metodi. Frontend aplikacije
...
Na sljedećoj slici može se vidjeti struktura frontend dijela. U okviru frontend dijela prikazani su samo odabrani dijelovi koda i komponente zbog njihove opsežnosti. U slučaju interesa za konkretnim dijelom, izvorni kod je dostupan na Github 
...
Funkcija askAssistant šalje korisničko pitanje AI asistentu putem POST zahtjeva na backend. Automatski generira i koristi jedinstveni ID sesije, 
...
Sljedeća funkcija dohvaća popis trenutno pohranjenih dokumenata sa servera. Koristi GET zahtjev prema /documents.
...
Ova funkcija dohvaća statistiku o dokumentima s rute /stats. Vraća podatke poput broja dokumenta, broja ulomka i vremena zadnjeg dodavanja. 
...
Sljedeća funkcija briše dokument prema nazivu koristeći DELETE zahtjev. Naziv datoteke se enkodira kako bi bio siguran za URL.
...
Sljedeća funkcija se koristi za slanje jednog ili više dokumenata (PDF/JSON). Kreira FormData i uključuje ID sesije. Ako server ne prihvati
...
Posljednja funkcija dohvaća sadržaj dokumenta (npr. JSON) za pregled. Koristi GET zahtjev prema /preview document. Vraća sadržaj u čitljivom 
...
About komponenta prikazuje sekciju s videozapisom i informacijama od gradu Lodzu. Koristi useContext i LanguageContext čime se dohvaćaju
prijevodi teksta na temelju trenutno odabranog jezika (translations.aboutPage). Nadalje ugrađeni iframe iz Google Drive-a prikazuje video o Lodzu.
...
Sljedeća datoteka postavlja kontekst za jezik aplikacije koristeći React Context API. Omogućuje da cijela aplikacija zna koji jezik je trenutno
...
jezik (useState('en')), dok se setLanguage koristi za promjenu jezika. Nadalje ako je odabrani jezik hr, koristi se hr.json, u suprotnom en.json.
Funckija toggleLanguage omogućuje promjenu jezika pozivom funkcije toggleLanguage('hr'). LanguageContext.Provider omogućuje da djeca komponente
...
Sljedeća komponenta predstavlja glavnu početnu stranicu aplikacije i prikazuje sve ključne sekcije. Koristi višekratne komponente poput Navbar,
...
Ova glavna React komponenta postavlja strukturu aplikacije koristeći BrowserRouter za upravljanje rutama. Omotana je u LanguageProvider kako
...
Sljedeći dio koda upravlja vektorskom bazom podataka koristeći FAISS i HuggingFaceEndpointEmbeddings. Omogućuje učitavanje vektorske baze podataka, dodavanje
...
Sljedeći dio koda dohvaća embeddinge i ako direktorij vectorstore postoji: učitava bazu, dodaje dokumente ako ih ima, i sprema ažuriranu bazu.
...
Sljedeći dio koda učitava vektorsku bazu podataka nakon čega filtrira sve dokumente čiji source nije jednak filename. Ako se broj dokumenata
...
, model koristi llama-3.1-8b-instant za brze odgovore i ključ se učitava iz .env datoteke. Nadalje se incijalizira memorija po korisniku i ako
...
pri svakom pitanju. Nadalje ako postoje dokumenti u FAISS bazi, pretvaraju se u retriver koji dohvaća najrelevantnije dijelove.
...
Ovdje se prvo provjeravaju jesu li datoteke poslane nakon čega se kreiraju privremene mape za pohranu dokumenata gdje će se dokumenti privremeno
...
Zatim se provjerava tip dokumenta i ako je pdf, koristi se PyPDFLoader iz LangChain-a za učitavanje sadržaja kao Document. Ako je .json, parsira se
...
\subsection{Rute}
Ova API ruta omogućuje korisniku da pošalje pitanje i jedinstveni ID sesije, a zatim dohvaća odgovor koristeći vektorsku bazu podataka i LLM model.
...
Sljedeća ruta omogućuje prikaz pregleda sadržaja .json datoteke s diska. Prvo provjerava postoji li tražena datoteka unutar mape Scraper/data, zatim
...
Ova API ruta vraća statistiku o trenutno učitanim dokumentima u bazi. Prvo dohvaća sve dokumente i filtrira one koji imaju metapodatke o vremenu
...
Zadnja API ruta omogućuje korisnicima da uploadaju PDF ili JSON dokumente putem forme. Primljeni dokumenti se obrađuju funkcije process documents
...
početnoj ideji rada ali nakon što je započet rad s AI dijelom, pojavila se potreba za dodavanjem ove stranice kako bi se bolje 
...
Na sljedećoj slici je prikazan dashboard s Groq platforme, konkretno za model llama-3.1-8b-instant. Prikazuju se grafovi s brojem zahtjeva i ukupnim brojem tokena u određenom vremenskom okviru.
...
Ova slika prikazuje tablicu s ograničenjima korištenja za različite modele unutar Groq platforme. Za svaki model prikazani su maksimalni brojevi upita u minuti i danu, kao i ograničenja na broj tokena.
