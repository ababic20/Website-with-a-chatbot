...
prvotno za sve studente koji dolaze u Łódź iz svojih država. Osim toga web stranica je naravno dostupna i namijenjena i za sve ostale zainteresirane posjetitelje. Web stranica sastoji se od 
...
RAG sustavu, te stranice s kontaktnom formom. Forma je namijenjena za pitanja na koja AI (eng. Artificial Inteligence) asistent nije znao odgovoriti.
...
korišten je HuggingFaceEndpointEmbeddings, također preko API ključa s platforme Hugging Face. Kao vektorska baza podataka korišten je FAISS (Facebook AI Similarity Search), 
...
\subsubsection{GPT-4}
GPT (eng. Generative Pre-trained Transformer) je veliki, multimodalni jezični model koji je sposoban prihvatiti tekstualne i slikovne ulaze te generirati teksutalne izlaze. Model je transformer
arhitekture, treniran je da predviđa sljedeći token u tekstu, a naknadna prilagodba poboljšava željeno ponašanje. U usporedbi s prethodnikom GPT-3.5, postiže bolje rezultate na zajedničkim NLP
...
Fino podešavanje kreće odabirom modela opće namjene koji već široko razumije jezik. Nadalje se priprema skup podataka koji je relevantan za određenu domenu (Npr. modeli opće namjene mogu imati npr. poteškoća s medicinskom terminologijom.)
...
svoje interne parametre kako bi se bolje uklopili uzorci u skupu podataka za fino podešavanje. Proces učenja više puta propušta te primjere kroz neurosnku mrežu,
...
Kada se uspoređuju pristupi RAG i finog podešavanja, postoji nekoliko ključnih čimbenika koji mogu pomoći pri odabiru metode koja je prikladnija za konkretni problem.
\begin{table}[h]
\centering
...
\hline
Razmatranje troškova & RAG obično zahtijeva niža početna ulaganja, kasniji troškovi za hosting baze i infrastrukturu za pretraživanje. & Fino podešavanje zahtijeva značajne početne računalne resurse za obuku, ali rezultira nižim troškovima po upitu. \\
\hline
Privatnost podataka & RAG drži podatke odvojeno u kontroliranim bazama podataka, što omogućuje brisanje podataka kada je to potrebno. & Podaci su ugrađeni direktno u model, što neke organizacije smatraju zabrinjavajućom aktivnošću. \\
\hline
Inženjerski resursi & Stručnost u području baza podataka i pretraživanja. & Strojno učenje, pristup GPU-u i iskustvo s obučavanjem. \\
\hline
Željena razina kontrole modela & Manje izravne kontrole, ali lakša ažuriranja i ispravke. & Potpuna kontrola nad ponašanjem modela i stilom izlaza. \\
\hline
\end{tabular}
...
Izbor između RAGA i finog podešavanja je više tehnička odluka. RAG se ističe kada su potrebne ažurne informacije ili kada su ograničeni resursi za obuku. Fino podešavanje je izvrsno za 
specijalizirane zadatke koji zahtjevaju stručnost u određenoj domeni. Postoji opcija hibridnog pristupa gdje je moguće kombinirati ove dvije metode. Krajnja odluka bi trebala biti usklađena
...
rezultatima modela. Uspjeh bilo kojeg pristupa ovisi ponajviše o kvaliteti podataka tijekom cijelog procesa. Loša kvaliteta podataka potkopava RAG i fino podešavanje. Nebitni ili zastarijeli
dokumenti smanjuju performanse RAG-a, dok krivo označeni ili pristrani podaci za obuku stvaraju nepuzadane fino podešene modele. Ovakvi problemi s kvalitetom podataka narušavaju performanse
umjetne inteligencije, što dovodi krajnje štete i narušavanja povjerenja korisnika \cite{montecarlo2023rag}. 

\subsection{Prednosti RAG sustava}
...
\subsection{Tipični slučajevi upotrebe}
...
\subsection{Izazovi RAG pristupa}
...
Prije svega, različiti izvori često donose različite informacije koje nisu uvijek usklađene. To može podrazumijevat kontradiktorne podatke, različite perspektive te redudantne podatke.
Model se suočava s izazovom kako inteligentno itegrirati te podatke bez stvaranja zbunjujućih ili netočnih odgovora. Ako model pogrešno spoji podatke iz nespojivih izvora podataka,
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{./assets/Multi hop.png}
...
\begin{table}[ht!]
\centering
\begin{tabular}{|p{3.7cm}|p{3.2cm}|p{3.7cm}|p{3.5cm}|}
\hline
...
Postavljen je na širinu 100 posto i visinu 400px. Za taj video bilo je potrebno odvojiti jedan dan kako bi se napisali i složili engleski titlovi
...
kada su slani upiti prema modelu. Osim toga moguće je pregledati i dnevnu/mjesečnu potrošnju u smislu financijskih sredstava. Isto tako kroz karticu Logs omogućen je uvid detaljniji opisa
zahtjeva koji je stigao na Groq (kada je kreiran, koji model je korišten, koji API key, status code, kolko input tokena i output je potrošeno i od kog usera)
